...
const Sidebar = () => {
  //@ts-expect-error its returning unkown type
  const { getUsers, users, isUsersLoading, selectedUser, setSelectedUser } =
    useChatStore();

  //@ts-expect-error eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      <div className="overflow-y-auto w-full py-3">
        ...
        {filteredUsers.length === 0 && (
          <div className="p-5 text-sm text-zinc-500">
            No users found{showOnlineOnly ? " online" : ""}.
          </div>
        )}
      </div>
    </aside>
